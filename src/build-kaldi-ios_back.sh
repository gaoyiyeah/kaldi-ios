...
default_ios_version=11.4
default_min_ios_version=9.0
default_macosx_version=10.13
...
case $target in
    iphone )
        ...
        # host=darwin-${default_ios_version}~iphone
        ;;
...
prefix=`pwd`/build/${target}-${IOS_VERSION}
...
# 刚刚注释 export CPPFLAGS="-miphoneos-version-min=${MIN_IOS_VERSION} -pipe -no-cpp-precomp -I$SDKROOT/usr/include"
# export CFLAGS="$CPPFLAGS -std=c++11 -arch ${arch} -isysroot $SDKROOT -isystem $SDKROOT/usr/include"
...
# 刚刚注释 export CFLAGS="$CPPFLAGS -std=c++11 -arch armv7 -arch arm64 -isysroot $SDKROOT -isystem $SDKROOT/usr/include"
# 刚刚注释 export CXXFLAGS="$CPPFLAGS -arch armv7 -arch arm64 -isysroot $SDKROOT -isystem $SDKROOT/usr/include"

export CPPFLAGS="-miphoneos-version-min=${MIN_IOS_VERSION} -pipe -no-cpp-precomp -I$SDKROOT/usr/include"
export CFLAGS="$CPPFLAGS -std=c++11 -arch armv7 -arch arm64 -isysroot $SDKROOT"
export CXXFLAGS="$CPPFLAGS -arch armv7 -arch arm64 -isysroot $SDKROOT"


export LDFLAGS="-miphoneos-version-min=${MIN_IOS_VERSION} -arch armv7 -arch arm64 -isysroot $SDKROOT -L$SDKROOT/usr/lib -L$DEVROOT/usr/lib"
